...
When `WORKFLOW_UI_REQUIRE_API_KEY=1`, add `Authorization: Bearer <WORKFLOW_UI_API_KEY>` for mutating `POST`/`PUT`. Localhost exemption is explicit opt-in via `WORKFLOW_UI_API_KEY_EXEMPT_LOCAL=1` for local-only development.
