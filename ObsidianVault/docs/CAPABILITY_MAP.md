...
- **Optional:** Set `WORKFLOW_UI_REQUIRE_API_KEY=1` and `WORKFLOW_UI_API_KEY`; mutating `POST`/`PUT` under `/api/` require header `Authorization: Bearer <key>` (unless `GET`/`/`). Set `WORKFLOW_UI_API_KEY_EXEMPT_LOCAL=1` only for explicit local-only development.
