...
group: harness-pending-tasks
color: rose
cssclasses:
  - vault-grp-harness-pending-tasks
  - vault-col-rose
---
...
**Cross-ref (OpenGrimoire observability hub):** [§ PENDING_OPENGRIMOIRE_OBSERVABILITY_HUB](#pending_opengrimoire_observability_hub) — operator probe surface + backlog to grow **OpenGrimoire as central hub** for internal monitoring, reflections, and AI operations telemetry (charter shipped: `OpenGrimoire/docs/plans/2026-04-23-opengrimoire-internal-monitoring-hub-charter.md`, **OG-OH-10** `done`).
...
| GH-DL3 | pending | **Two-desk cable management + ergonomics:** Desk A (~3×3 ft + TV + power strip + laptop + liminal clutter); Desk B (painting + local-proto + Drobo) — raceways/trays, **monitor vs keyboard** (comfortable parallel / gentle open V). **Operator layout:** attic **roof slope**; monitors on **raised surface** with **two arms** on end of **partition wall**, rotated toward user; **no front overhang** on desk—**flat side boards** → **edge clamps won’t work**; preferred fix = **keyboard tray/arm** via **through-bolt or grommet** (or screw-fixed rail into underside), not clamp-only. One printed win per desk + GearHead note. **What AI needs:** wide **photo** (keyboard zone + monitor arms + desk edge), optional rough **depth/height** of usable underside. | [Cable Raceway — timbarnes](https://www.printables.com/model/516164-cable-raceway), [Cables raceway (duct) — Gregzy](https://www.printables.com/model/126370-cables-raceway-duct), [Under desk cable management — dsw73](https://www.printables.com/model/701810-under-desk-cable-management), [Parametric raceway — elementalvoid](https://www.printables.com/model/1255666-cable-raceway-adjustable-size-comb-screw-holes); [DROBO_AI_STORAGE.md](../../local-proto/docs/DROBO_AI_STORAGE.md) |
| GH-DL4 | pending | **Basement — clear/clean → hardware BOM → servers online:** (1) Floor path, dust, moisture, trip hazards; (2) inventory table (hostname intent, CPU, RAM, disks, GPU, PSU W, NIC, OS); (3) network + labeled PDUs; (4) boot order + DHCP reservation or static IP notes. **Hardware context:** PowerEdge-class (and related) boxes—operator will paste **BOM** next session. **Blocked:** until first clear-out session scheduled. | [HARDWARE.md](../../local-proto/docs/HARDWARE.md); optional future `docs/superpowers/specs/` home-lab charter |
...
| ID | Status | Task | Spec / Link |
|----|--------|------|-------------|
| OGAN-09 | deferred | **Product decision + API (optional):** Persist operator viz prefs (theme, autoplay, colors) behind authenticated routes if agent parity is required without browser. | §P9 |
| OGAN-10 | deferred | **Backlog / future:** Versioned chart-spec JSON + thin renderer (prompt-native path); only if product commits. | §P10 |

### Post-OGAN PR / merge hygiene (2026-04-19 session — ce-review + chat)

| ID | Status | Task | Spec / Link |
|----|--------|------|-------------|
| OG-PR-4 | pending | **Operator:** compound **`/ce-review`** on the active PR branch (not automated in agent session) + **AN1** formal **waive** / **`split_done`** per policy. **Doc (2026-04-21):** [verification log](../../OpenGrimoire/docs/audit/evidence/og-system2-mcp-wave/CE_REVIEW_DEFERRAL.md#og-pr-closure-verification-2026-04-21) + [OG-PR-4 / AN1 status](../../OpenGrimoire/docs/audit/evidence/og-system2-mcp-wave/CE_REVIEW_DEFERRAL.md#og-pr-4-status-2026-04-21) — `master` automations + Cursor **`mode:report-only`** narrative review recorded; **PR-branch compound `/ce-review`** and **AN1** harness closure still **open**. | [AGENT_NATIVE_AUDIT_OPENGRIMOIRE.md](../../OpenGrimoire/docs/AGENT_NATIVE_AUDIT_OPENGRIMOIRE.md); [CE_REVIEW_DEFERRAL.md](../../OpenGrimoire/docs/audit/evidence/og-system2-mcp-wave/CE_REVIEW_DEFERRAL.md) |
...
| ID | Status | Task | Spec / Link |
|----|--------|------|-------------|
| OG-AUDIT-02 | pending | **Rules hygiene:** When importing community Cursor rules or skills, run **security-audit-rules** checklist on those files only (workspace `.cursor/skills/security-audit-rules/SKILL.md` when present). | [gui data-viz — OG-AUDIT-02 checklist](../../OpenGrimoire/docs/audit/gui-2026-04-16-opengrimoire-data-viz.md#post-implementation-process-checklist-release-gate) · Cursor skill **security-audit-rules** |
| OG-GUI-AUDIT-01 | deferred | **System 1 — A2UI catalog:** When first agent-generated/declarative admin ships, add catalog semantics + props map (MiscRepos A2UI guidance or OG equivalent). **Related:** **OA-OG-5** deferred — same product gate (no declarative admin surface yet). | [gui survey §5 A2UI / catalog](../../OpenGrimoire/docs/audit/gui-2026-04-16-opengrimoire-survey.md#5-a2ui-catalog) |
...
| OG-DV-UI-02 | pending | **A2UI / selectors:** Extend `data-region` / `data-testid` to `/constellation` loading shell and Zustand-driven demo controls. | [gui data-viz §5](../../OpenGrimoire/docs/audit/gui-2026-04-16-opengrimoire-data-viz.md); **OGAN-15/17** shipped axe + selector doc — **remaining** UI instrumentation per §5 |
...
**Already landed in OpenGrimoire (track for regressions only; not backlog rows):** stable `data-testid` + list→detail→delete E2E; axe on `/admin/observability` (+ detail with seeded run); `npm run verify` → `verify:operator-probes-auth`; optional **`OPERATOR_PROBE_ADMIN_SECRET`** + **`x-operator-probe-admin-key`** for admin probe list/detail/delete without browser session (**OG-OH-06**); ingest **503** “misconfigured” omits `access_denied` to cut scanner noise; optional **`ACCESS_DENIED_INVALID_SECRET_*`** envs to throttle **401** **`invalid_secret`** `access_denied` lines (**OG-OH-09**); “Actions” column header for table a11y; `CONTRIBUTING.md` / `DEPLOY_AND_VERIFY.md` verify chain; **OG-OH-11 … 13** vision slice closed **2026-04-23** — see [Completed-Tasks § PENDING_OPENGRIMOIRE_OBSERVABILITY_HUB](Completed-Tasks.md#pending_opengrimoire_observability_hub).

**Open OG-OH rows:** none. New hub work: add fresh labeled harness IDs.
...
| ID | Status | Task | Spec / Link |
|----|--------|------|--------------|
| IC-1 | pending | **Intent / collaboration acceleration:** **Session prime** (≤10 lines or fixed template): active task IDs (e.g. **GH-DL\***), venue, “read `handoff_latest ## Next` first,” money/collab focus if any. **End session** (≤2 min): bump **`## Next`** when automation should see it, or one GearHead note if only meat-space moved. **Weekly:** 3-bullet retro (worked / drifted / one SSOT fix). **Goal:** less re-derivation, same loyalty-quality thread. | [HITL_CONNECTIVITY](../../local-proto/docs/HITL_CONNECTIVITY.md); `handoff_latest.md` |
| BMI-1 | pending | **EEG → interface (non-invasive, solo-attainable path):** Re-read [bmi-neuroscience README](../../docs/bmi-neuroscience/README.md). **Phase A** — headset ergonomics + when you will record (same time of day helps). **Phase B** — raw stream locally (LSL / [Muse SDK](https://choosemuse.com/pages/developers) / [amused-py](https://github.com/Amused-EEG/amused-py) per README). **Phase C** — one derived signal (e.g. bandpower / focus proxy) with **simple** ground-truth labels before any UI hook. **Defer** “control Cursor / type by thought” until C is boring-reliable. **Human gate:** [friction matrix](../../docs/bmi-neuroscience/2026-03-14-friction-matrix.md). | [virtual-project-charter.md](../../docs/bmi-neuroscience/2026-03-14-virtual-project-charter.md), [oss-repo-evaluation.md](../../docs/bmi-neuroscience/2026-03-14-oss-repo-evaluation.md) |
| BMI-2 | pending | **EEG headset class — operator chose consumer BLE first:** **Muse-class** as MVP path; **OpenBCI / Galea** tier remains comparison for later high-density work (see [Galea](https://galea.co/) vs [Muse dev paths](../../docs/bmi-neuroscience/README.md)). **Deliverable:** confirm **exact Muse model** owned + **one paragraph** “why Muse first” in GearHead or adhoc; comfort window (minutes/session). | [archive index](../../docs/bmi-neuroscience/archive/2026-04-19-downloaded-papers-and-links-index.md); [PMC7876403](https://pmc.ncbi.nlm.nih.gov/articles/PMC7876403/) |
| BMI-3 | pending | **MVP — Muse → local compute (no cloud on EEG):** **Phase 1** BLE → **LSL** on LAN host ([Muse SDK](https://choosemuse.com/pages/developers) / [amused-py](https://github.com/Amused-EEG/amused-py) / OpenMuse per README). **Phase 2** persist **.xdf** or **EDF** under operator-controlled path (Drobo/NAS ok). **Phase 3** offline **MNE** (PSD / sanity ERP if marked). **Defer** remote fine-tuning of weights on raw brain data. **Exit:** 60 s recording + file hash logged in GearHead note. | [bmi-neuroscience README](../../docs/bmi-neuroscience/README.md); [downloaded papers index](../../docs/bmi-neuroscience/archive/2026-04-19-downloaded-papers-and-links-index.md) § D |
...
| ID | Status | Task | Spec / Link |
|----|--------|------|-------------|
| AT-TRIBE1 | pending | **Meta TRIBE v2 → in-silico media / brain-response research:** Official link archive + AI Trends raw **`.cursor/state/ai_trends/raw/2026-04-19/manual_bmi_tribe_v2_01.txt`** under **MiscRepos** (indexed). **Next:** run `ai_trends_stack_intel.py --date 2026-04-19 --prefix manual_bmi` **with Ollama** for stack-facing recommendations; optional `scp_analyze_ai_trends`. **Caution:** TRIBE v2 predicts **population fMRI** patterns (CC BY-NC), not your **EEG** trace—pair with **BMI-1** as complementary simulation, not ground truth for BCI. **Commercial:** verify BY-NC vs your monetization path with counsel if needed. | [TRIBE v2 archive](../../docs/bmi-neuroscience/2026-04-19-tribe-v2-official-documentation-archive.md); [AI_TRENDS_MCP.md](../../local-proto/docs/AI_TRENDS_MCP.md) § Manual raw drop |
