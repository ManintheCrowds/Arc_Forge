...
**Cross-ref (OpenGrimoire observability hub):** [§ PENDING_OPENGRIMOIRE_OBSERVABILITY_HUB](#pending_opengrimoire_observability_hub) — operator probe surface + charter vision slice **OG-OH-11 … 13** `done` (2026-04-23); charter `OpenGrimoire/docs/plans/2026-04-23-opengrimoire-internal-monitoring-hub-charter.md`.
...
**Already landed in OpenGrimoire (track for regressions only; not backlog rows):** stable `data-testid` + list→detail→delete E2E; axe on `/admin/observability` (+ detail with seeded run); `npm run verify` → `verify:operator-probes-auth`; optional **`OPERATOR_PROBE_ADMIN_SECRET`** + **`x-operator-probe-admin-key`** for admin probe list/detail/delete without browser session (**OG-OH-06**); ingest **503** “misconfigured” omits `access_denied` to cut scanner noise; optional **`ACCESS_DENIED_INVALID_SECRET_*`** envs to throttle **401** **`invalid_secret`** `access_denied` lines (**OG-OH-09**); “Actions” column header for table a11y; `CONTRIBUTING.md` / `DEPLOY_AND_VERIFY.md` verify chain; **OG-OH-11** Operations header nav on `/admin/*` + `/admin` Operations hub callout; **OG-OH-12** correlation spike plan doc; **OG-OH-13** verify gates in charter + **AGENT_INTEGRATION**.

**Open OG-OH rows:** none (vision slice closed **2026-04-23**). New hub execution should add fresh labeled IDs. Closed rows: [completed_tasks.md](completed_tasks.md#pending_opengrimoire_observability_hub).
